...
def split_audio(audio_file_path, chunk_duration=30):
    ...
    try:
        ...
        for i in range(0, len(audio), chunk_length_ms):
            ...
            chunk_path = f"{audio_file_path[:-4]}_chunk{i//chunk_length_ms}.wav"
    ...
    try:
        ...
        for chunk_path in chunks:
            ...
            config = speech_v1p1beta1.RecognitionConfig(
                ...
                sample_rate_hertz=44100,  # Changed to match WAV header
                language_code="en-US",
            ...
            for result in response.results:
                if result.alternatives:
                    ...
                    print(f"Chunk confidence: {alternative.confidence}")
            ...
            if cleanup:
                try:
                    ...
                    print(f"Error cleaning up chunk: {str(e)}")
        ...
        duration = all_words[-1].end_time.total_seconds() - all_words[0].start_time.total_seconds()
        wpm = (len(all_words) / duration) * 60 if duration > 0 else 0
        avg_confidence = sum(word.confidence for word in all_words) / len(all_words)

        print(f"Processed total {len(all_words)} words with average confidence {avg_confidence}")
        ...
        if cleanup and os.path.exists(audio_file_path):
            try:
                os.remove(audio_file_path)
                print(f"Cleaned up original audio file: {audio_file_path}")
            except Exception as e:
                print(f"Error cleaning up file: {str(e)}")
...
def extract_audio_from_video(video_path):
    ...
    try:
        ...
        # Write audio to WAV file with mono audio
...
#curl -X POST -F "video=@/Users/lv/Downloads/test.mp4" http://localhost:5000/analyze-video